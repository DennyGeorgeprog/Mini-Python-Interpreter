...
//checks if is an identifier and increments token
...
//checks if is an operator and increments token
...
//checks if is an string and increments token
...
//checks if is a print call for lists
...
//checks if is a list assignment
...
//checks if is a list concatenation
...
//checks if its an assignment of a function call
...
bool Parser::isAssignmentFunc(std::vector<Token> &line,std::map<std::string,varInfo> funcmap){
  if(isListAssignmentFunc(line) || isListConcatAssignmentFunc(line,funcmap) || isCommentedLineFunc(line)) {
    return false;
  }
  return true;
}
...
//checks if is an return statement
...
//allows for function calls in assignments
...
void Parser::parse(std::vector<Token> &tokens){
  ...
  while (mCurrTok != mEndTok){
    ...
    //parses list assignment
      ...
      std::map<std::string, varInfo>::iterator it, itCurr;
      std::string currVarName;
      if (variables.find(line[0].mText) == variables.end()){
        varInfo currVar = (varInfo){.value = 0, .values = {}, .isList = true};
        variables.insert({line[0].mText, currVar});
    ...
    //parses list updates
      ...
      if(line[2].mText == ":" || line[3].mText == ":") {
        ...
        int listIndex = std::stoi(line[2].mText);
        it = variables.find(line[0].mText);
        if (it == variables.end()){
          std::cout << "ERROR: No such variable defined" << std::endl;
          exit(0);
    ...
    //parses functions assigned to variables
...
int count = 0;
  //checks if the line is tabbed and how far tabbed it is
...
std::vector<int> ln3;
for (int i = 0; i < tokens.size(); i++) {
  if (tokens[i].isTabed == true) {
    ln.push_back(tokens[i].mLineNumber);
  }
  if (tokens[i].tabNum > 0) {
    ln2.push_back(tokens[i].tabNum);
    ln3.push_back(tokens[i].mLineNumber);
  }
}

for (int i = 0; i < ln3.size(); i++) {
  for (int j = 0; j < tokens.size(); j++) {
    if (tokens[j].mLineNumber == ln3[i]) {
      tokens[j].tabNum = ln2[i];
    }
  }
}
...
for (int i = 0; i < ln.size(); i++) {
  for (int j = 0; j < tokens.size(); j++) {
    if (tokens[j].mLineNumber == ln[i]) {
      tokens[j].isTabed = true;
    }
  }
}
...
for(std::vector<Token>::iterator it=tokens.begin();it!=tokens.end();) {
 ...
 for(Token currTok:tokens) {
   currTok.debugPrint();
 }
  //parses the tokenized tokens
  Parser parser;
  parser.parse(tokens);
